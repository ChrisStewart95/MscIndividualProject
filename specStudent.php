<?php
	...
	//Using POST to get the specific friend selected from myFriends.php
	$studentID = $_POST['hiddenStudentID'] ?? '';
	// If value is not being set for whatever reason, like a refresh, send back to mystudents.php
	if ($studentID == ''){
	...
	$readEasyResults = "SELECT * FROM AC_Scores WHERE StudentID = '$studentID' AND difficulty = 'Easy'";
	...
	$readInterResults = "SELECT * FROM AC_Scores WHERE StudentID = '$studentID' AND difficulty = 'Intermediate'";
	// Querying the SQL database
	...
	$readAdvResults = "SELECT * FROM AC_Scores WHERE StudentID = '$studentID' AND difficulty = 'Advanced'";
	// Querying the SQL database
...
<body id="baseBackground">
	<div class="container-fluid">
		<div class="row" id="bannerTutor">
			...
			<div class="col-1 btnCenter">
				<a class="btn btn-light" href="myStudents.php"><strong>Back</strong></a>
			</div>
	...
	<?php
							...
							<div class='col-12 text-center'>
								<div class='alert alert-warning fade show' role='alert'>
									...
									<form action='specStudent.php' method='POST' enctype='multipart/form-data'>
										...
										<button type='submit' name='submitBtnAccept' class='btn btn-light'><strong>Yes, remove this student</strong></button>
										
									</form>
									
									<form action='specStudent.php' method='POST' enctype='multipart/form-data'>
										<input type = 'hidden' value='$studentID' name='hiddenStudentID'>
										<button type='submit' name='submitBtnDecline' class='btn btn-light'><strong>No, keep this student</strong></button>
									</form>
								</div>
							</div>
						</div>
		...
		if(isset($_POST['submitBtnAccept'])){
			...
			$update = "UPDATE AC_Users SET tutorship = NULL WHERE userid = '$studentID'";
			...
			$delete = "DELETE FROM AC_TutorToStudent WHERE tutorID = '$dynamicUserID' AND studentID = '$studentID'";
		...
		if(isset($_POST['submitBtnDecline'])){
			header("Location: specStudent.php?specificMyStu=$studentID");
		}
			...
			<?php 
				while($row1 = $result->fetch_assoc()){
					...
					echo 	"<div class='col-12 text-center' id='formWidth'>
									...
									<br>
									<h3>$studentName</h3>
									<img class='specStuImg' src='../imgs/$studentPic'>
							...
							echo "<div class='col-12 text-center'>
									<div>
										<form action='specStudent.php' method='POST' enctype='multipart/form-data'>
											<input type = 'hidden' value='$studentID' name='hiddenStudentID'>
											<button type='submit' name='submitBtn' class='btn btn-danger'><i class='fas fa-times'></i><strong> Remove Student</strong></button>
										</form>
									</div>
								</div>";
